#!/usr/bin/env python3
"""
Confluence audit records -> Wazuh local JSON ingestion

Polls the Confluence audit endpoint and appends one flat JSON record per new
audit event to a local file that Wazuh ingests via a <localfile> block.

  Default API: GET {CONFLUENCE_BASE_URL}/wiki/rest/api/audit
  Auth:        Basic user:api_token, or Bearer token/PAT

Writes JSON lines to:
  /var/ossec/logs/confluence/confluence-events.json

State file:
  /var/ossec/queue/confluence/confluence-events-state.json

Config (KEY=VALUE) is read from:
  /var/ossec/etc/confluence-events.env

Recommended execution:
  sudo -u wazuh /var/ossec/integrations/confluence_events.py

Design notes
------------
Every emitted event is flattened into collision-safe top-level "conf_*" scalar
fields. Nested Confluence audit structures such as author, affectedObject,
changedValues, and associatedObjects are summarized into scalar fields only.
This avoids shared wazuh-alerts-* index mapping conflicts caused by object/scalar
collisions.
"""
...
DEFAULT_ENV_FILE = "/var/ossec/etc/confluence-events.env"
DEFAULT_STATE_FILE = "/var/ossec/queue/confluence/confluence-events-state.json"
DEFAULT_OUTPUT_FILE = "/var/ossec/logs/confluence/confluence-events.json"
DEFAULT_AUDIT_PATH = "/wiki/rest/api/audit"
...
def format_query_time(dt: datetime, mode: str) -> str:
    ...
    if mode == "epoch_ms":
        return str(to_epoch_millis(utc))
    if mode == "iso":
...
def get_config() -> Dict[str, Any]:
    env_file = os.environ.get("CONFLUENCE_ENV_FILE", DEFAULT_ENV_FILE)
    load_env_file(env_file)

    api_token = read_secret("CONFLUENCE_API_TOKEN", "CONFLUENCE_API_TOKEN_FILE")
    bearer_token = read_secret("CONFLUENCE_BEARER_TOKEN", "CONFLUENCE_BEARER_TOKEN_FILE") or read_secret(
        "CONFLUENCE_PAT", "CONFLUENCE_PAT_FILE"
    )
    bearer_token = strip_bearer(bearer_token)

    auth_mode = os.environ.get("CONFLUENCE_AUTH_MODE", "auto").strip().lower()
    if auth_mode == "auto":
        if bearer_token:
            auth_mode = "bearer"
        elif os.environ.get("CONFLUENCE_USERNAME", "").strip() and api_token:
            auth_mode = "basic"
        elif api_token:
            auth_mode = "bearer"
    ...
    date_format = os.environ.get("CONFLUENCE_DATE_FORMAT", "date").strip().lower()
    if date_format not in {"date", "iso", "epoch_ms"}:
        date_format = "date"

    return {
        "env_file": env_file,
        "base_url": os.environ.get("CONFLUENCE_BASE_URL", "").strip().rstrip("/"),
        "audit_path": normalize_path(os.environ.get("CONFLUENCE_AUDIT_PATH", DEFAULT_AUDIT_PATH).strip()),
        "auth_mode": auth_mode,
        "username": os.environ.get("CONFLUENCE_USERNAME", "").strip(),
        "api_token": api_token,
        "bearer_token": bearer_token,
        "state_file": os.environ.get("CONFLUENCE_STATE_FILE", DEFAULT_STATE_FILE),
        "output_file": os.environ.get("CONFLUENCE_OUTPUT_FILE", DEFAULT_OUTPUT_FILE),
        "backfill_hours": parse_int(
            os.environ.get("CONFLUENCE_BACKFILL_HOURS"), DEFAULT_BACKFILL_HOURS, minimum=1, maximum=24 * 90,
        ),
        "limit": parse_int(
            os.environ.get("CONFLUENCE_LIMIT"), DEFAULT_LIMIT, minimum=1, maximum=1000,
        ),
        "max_pages": parse_int(
            os.environ.get("CONFLUENCE_MAX_PAGES"), DEFAULT_MAX_PAGES, minimum=1, maximum=100,
        ),
        "http_timeout": parse_int(
            os.environ.get("CONFLUENCE_HTTP_TIMEOUT"), DEFAULT_HTTP_TIMEOUT, minimum=5, maximum=300,
        ),
        "sleep_between_pages": parse_float(
            os.environ.get("CONFLUENCE_SLEEP_BETWEEN_PAGES"), DEFAULT_SLEEP_BETWEEN_PAGES, minimum=0.0,
        ),
        "lookback_seconds": parse_int(
            os.environ.get("CONFLUENCE_LOOKBACK_SECONDS"), DEFAULT_LOOKBACK_SECONDS, minimum=0, maximum=24 * 3600,
        ),
        "seen_retention_hours": parse_int(
            os.environ.get("CONFLUENCE_SEEN_RETENTION_HOURS"), DEFAULT_SEEN_RETENTION_HOURS, minimum=1, maximum=24 * 14,
        ),
        "include_end_date": parse_bool(os.environ.get("CONFLUENCE_INCLUDE_END_DATE"), default=True),
        "date_format": date_format,
        "search_string": os.environ.get("CONFLUENCE_SEARCH_STRING", "").strip(),
        "extra_query": os.environ.get("CONFLUENCE_EXTRA_QUERY", "").strip(),
        "debug": parse_bool(os.environ.get("CONFLUENCE_DEBUG"), default=False),
    }
...
def load_state(state_file: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            pass
        eprint(f"Confluence: state file was unreadable, moved aside if possible: {exc}")
        return {"version": 1, "stream": {}}
...
def http_get_json(url: str, config: Dict[str, Any]) -> Tuple[int, Dict[str, Any], str]:
    headers = {
        ...
        "User-Agent": "wazuh-confluence-audit/1.0",
    }
    ...
    if config["debug"]:
        eprint(f"Confluence DEBUG: GET {url}")
...
def parse_confluence_datetime(value: Any) -> Optional[datetime]:
    if value is None:
        return None
...
def event_time(item: Dict[str, Any]) -> Optional[datetime]:
    value = first_present(item, ("creationDate", "created", "timestamp", "time", "eventTime", "date"))
    return parse_confluence_datetime(value)
...
def stable_event_id(item: Dict[str, Any]) -> str:
    ...
    stable = {
        "creationDate": first_present(item, ("creationDate", "created", "timestamp", "time", "eventTime", "date")),
        "summary": first_present(item, ("summary", "action", "eventName", "name")),
        "category": first_present(item, ("category", "eventCategory")),
        "author": first_present(author, ("accountId", "userKey", "username", "displayName")) if isinstance(author, dict) else None,
        "remoteAddress": first_present(item, ("remoteAddress", "remote_address", "ipAddress")),
        "affectedObject": item.get("affectedObject"),
        "changedValues": item.get("changedValues"),
        "associatedObjects": item.get("associatedObjects"),
    }
...
def extract_records(body: Any) -> List[Dict[str, Any]]:
    ...
    for key in ("results", "records", "events", "data", "values"):
        value = body.get(key)
...
def summarize_changed_values(values: Any) -> Dict[str, str]:
    ...
    for entry in values:
        if not isinstance(entry, dict):
            continue
        field = scalar_string(first_present(entry, ("name", "fieldName", "field")), limit=100)
        old_value = scalar_string(first_present(entry, ("oldValue", "changedFrom", "from")), limit=80)
        new_value = scalar_string(first_present(entry, ("newValue", "changedTo", "to")), limit=80)
        if field:
            fields.append(field)
    ...
    out: Dict[str, str] = {}
    if fields:
        out["conf_changed_fields"] = ",".join(sorted(set(fields)))
    if changes:
        out["conf_changed_values"] = "; ".join(changes[:10])
    out["conf_changed_count"] = str(len(values))
    return out


def summarize_associated_objects(values: Any) -> Dict[str, str]:
    if not isinstance(values, list):
        return {}
    ...
    for entry in values:
        if not isinstance(entry, dict):
            continue
        object_type = scalar_string(first_present(entry, ("objectType", "typeName", "type")), limit=80)
    ...
    out: Dict[str, str] = {}
    if types:
        out["conf_associated_types"] = ",".join(sorted(set(types)))
    if names:
        out["conf_associated_objects"] = ",".join(names[:20])
    out["conf_associated_count"] = str(len(values))
    return out


def flatten_event(item: Dict[str, Any], config: Dict[str, Any], event_id: Optional[str] = None) -> Dict[str, Any]:
    """Convert a raw Confluence audit record into a flat conf_* record."""
    record_id = event_id or stable_event_id(item)
    created_raw = first_present(item, ("creationDate", "created", "timestamp", "time", "eventTime", "date"))
    created_dt = parse_confluence_datetime(created_raw)
    summary = first_present(item, ("summary", "action", "eventName", "name"))
    category = first_present(item, ("category", "eventCategory"))

    out: Dict[str, Any] = {
        "conf_integration": INTEGRATION_NAME,
        "conf_event_type": EVENT_TYPE,
        "conf_ingested_at": to_rfc3339(utc_now()),
        "conf_event_id": record_id,
    }

    host = urllib.parse.urlparse(config["base_url"]).netloc
    if host:
        out["conf_site_host"] = host

    if created_raw is not None:
        out["conf_creation_date"] = scalar_string(created_raw, limit=80)
    if created_dt:
        out["conf_created_utc"] = to_rfc3339(created_dt)
    if summary:
        out["conf_summary"] = scalar_string(summary, limit=300)
        action = slugify_action(summary)
        if action:
            out["conf_action"] = action
    if category:
        out["conf_category"] = scalar_string(category, limit=160)

    for source_key, target_key in (
        ("description", "conf_description"),
        ("remoteAddress", "conf_src_ip"),
        ("remote_address", "conf_src_ip"),
        ("ipAddress", "conf_src_ip"),
        ("sysAdmin", "conf_sys_admin"),
        ("superAdmin", "conf_super_admin"),
    ):
    ...
    if isinstance(author, dict):
        for source_key, target_key in (
            ("accountId", "conf_author_account_id"),
            ("userKey", "conf_author_user_key"),
            ("username", "conf_author_username"),
            ("displayName", "conf_author_name"),
            ("publicName", "conf_author_public_name"),
            ("type", "conf_author_type"),
            ("accountType", "conf_author_account_type"),
            ("externalCollaborator", "conf_author_external_collaborator"),
            ("isExternalCollaborator", "conf_author_is_external_collaborator"),
            ("isGuest", "conf_author_is_guest"),
        ):
            value = author.get(source_key)
            if value is not None and target_key not in out:
                out[target_key] = scalar_string(value, limit=300)

    affected = item.get("affectedObject")
    if isinstance(affected, dict):
        for source_key, target_key in (
            ("name", "conf_affected_name"),
            ("objectType", "conf_affected_type"),
            ("typeName", "conf_affected_type"),
            ("id", "conf_affected_id"),
        ):
            value = affected.get(source_key)
            if value is not None and target_key not in out:
                out[target_key] = scalar_string(value, limit=300)

    out.update(summarize_changed_values(item.get("changedValues")))
    out.update(summarize_associated_objects(item.get("associatedObjects")))
...
def build_url(config: Dict[str, Any], start: int, from_dt: datetime, to_dt: datetime) -> str:
    params: Dict[str, str] = {
        "start": str(start),
        "limit": str(config["limit"]),
        "startDate": format_query_time(from_dt, config["date_format"]),
    }
    if config["include_end_date"]:
        params["endDate"] = format_query_time(to_dt, config["date_format"])
    if config["search_string"]:
        params["searchString"] = config["search_string"]
...
def poll(config: Dict[str, Any], state: Dict[str, Any]) -> int:
    ...
    while pages < config["max_pages"]:
        pages += 1
        url = build_url(config, start, from_dt, now)
        status, body, raw = http_get_json(url, config)

        if status == 401:
            eprint("Confluence: HTTP 401 Unauthorized. Check credentials and authentication mode.")
            eprint(raw)
            had_error = True
            break
        if status == 403:
            eprint("Confluence: HTTP 403 Forbidden. The account needs Confluence Administrator global permission.")
            eprint(raw)
            had_error = True
            break
        if status == 400:
            eprint("Confluence: HTTP 400 Bad Request. Check CONFLUENCE_AUDIT_PATH and CONFLUENCE_DATE_FORMAT.")
            eprint(f"Confluence: URL={url}")
            eprint(raw)
            had_error = True
            break
        if status == 429:
            eprint("Confluence: HTTP 429 rate limited. Stopping this run; will retry next interval.")
            had_error = True
            break
        if status < 200 or status >= 300:
            eprint(f"Confluence: HTTP {status}: {raw}")
            had_error = True
            break
        ...
        if config["debug"]:
            ...
            eprint(f"Confluence DEBUG: page={pages} start={start} items={len(records)} size={size}")
...
def validate_config(config: Dict[str, Any]) -> Optional[str]:
    if not config["base_url"] and not str(config["audit_path"]).startswith(("http://", "https://")):
        return f"missing CONFLUENCE_BASE_URL in {config['env_file']}."
    if config["auth_mode"] == "basic":
        if not config["username"] or not config["api_token"]:
            return (
                f"CONFLUENCE_AUTH_MODE=basic requires CONFLUENCE_USERNAME and "
                f"CONFLUENCE_API_TOKEN in {config['env_file']}."
            )
        return None
    if config["auth_mode"] == "bearer":
        if not config["bearer_token"]:
            return (
                f"CONFLUENCE_AUTH_MODE=bearer requires CONFLUENCE_BEARER_TOKEN "
                f"or CONFLUENCE_PAT in {config['env_file']}."
            )
        return None
    return "CONFLUENCE_AUTH_MODE must be auto, basic, or bearer."
...
def main() -> int:
    ...
    try:
        ...
        eprint(f"Confluence: unhandled error: {exc}")
        return 1

    if config["debug"]:
        eprint(f"Confluence DEBUG: emitted_total={total}")
    return 0
